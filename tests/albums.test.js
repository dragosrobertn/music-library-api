...
describe("/albums", () => {
  ...
  describe("POST /artists/:artistId/albums", () => {
      ...
      request(app)
        ...
        .then((res) => {
          ...
          Album.findByPk(res.body.id, { raw: true })
            .then((album) => {
              expect(album.name).to.equal("I'm Blessed");
    ...
    describe("with albums in the database", () => {
      ...
      beforeEach((done) => {
        Promise.all([
          Album.create({ name: "I'm Blessed", year: 2011 }),
          Album.create({ name: "Your Grace Finds Me", year: 2013 }),
          Album.create({ name: "Rehab", year: 2010 }),
        ]).then((documents) => {
          albums = documents;
          done();
        });
